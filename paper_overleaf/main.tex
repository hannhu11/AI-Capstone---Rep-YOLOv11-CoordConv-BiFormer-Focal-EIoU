...
\title{Rep-YOLO11s: Structural Re-Parameterization and Spatial Coordinate Encoding for Real-Time Safety Helmet Detection in Construction Surveillance}
...
\begin{abstract}
Personal Protective Equipment (PPE) compliance monitoring via closed-circuit television (CCTV) streams is critical for workplace safety in modern construction engineering. However, real-time automated safety helmet detection faces severe operational challenges, including distant tiny helmet targets ($<20 \times 20$ pixels), severe intra-frame class imbalance (up to $1:12$ helmet-to-person ratio), multi-scale occlusions, and background clutter. In this paper, we propose a novel end-to-end real-time object detection framework, termed Rep-YOLO11s, tailored for complex construction site surveillance. Our method seamlessly integrates four key technical innovations: (1) Structural Re-parameterization (RepConv) multi-branch blocks that enrich feature representation during training and algebraically collapse into a single $3\times3$ convolution during deployment for zero latency overhead; (2) Spatial Coordinate Convolution (CoordConv) to inject explicit position maps into feature channels, resolving spatial positional ambiguity; (3) Bi-Level Routing Attention (BiFormer) to dynamically filter irrelevant spatial tokens and focus compute on distant, occluded helmet contours; and (4) Focal EIoU Loss for bounding box regression, which simultaneously optimizes overlap, center distance, aspect ratio, and hard-sample weighting. Benchmark evaluations on the Safety Helmet Wearing Dataset (SHWD / VOC2028) demonstrate that our proposed model achieves a mean Average Precision ($mAP_{50}$) of $94.83\%$ and $mAP_{50-95}$ of $62.54\%$. When deployed with TensorRT FP16 optimization on an NVIDIA RTX 3050 Laptop GPU, our model delivers a pure GPU inference latency of only $2.14\text{~ms}$ ($467.3\text{~FPS}$) and an end-to-end RTSP pipeline speed of $65\text{--}95\text{~FPS}$, outperforming recent state-of-the-art architectures in both accuracy and operational throughput.
\end{abstract}

\begin{IEEEkeywords}
Safety Helmet Detection, Structural Re-parameterization, Bi-Level Routing Attention, CoordConv, Focal EIoU Loss, Real-Time Edge AI, Construction Safety Surveillance.
\end{IEEEkeywords}
...
\begin{itemize}
    ...
    \item \textbf{Focal EIoU Loss Optimization}: We formulate box regression using Focal EIoU Loss, decomposing bounding box error into overlap, center distance, width, and height metrics while dynamically re-weighting hard occluded bounding boxes to accelerate convergence.
    \item \textbf{Rigorous Hardware Validation}: We conduct extensive empirical evaluations on the public SHWD dataset. Our proposed model achieves $94.83\%$ $mAP_{50}$ with a pure GPU inference latency of $2.14\text{~ms}$ ($467.3\text{~FPS}$) on TensorRT FP16, outperforming existing state-of-the-art models.
\end{itemize}
...
YOLO11s & 9.40 & 21.5 & 94.74 & \underline{62.54} & 94.06 & \underline{90.35} & 0.9073 & 6.52 & 153.3 \\
\midrule
EC-YOLOv8~\cite{ecyolov8_2024} & 3.48 & 9.2 & 95.70 & 74.60 & \textemdash & \textemdash & \textemdash & 5.80 & 172.4 \\
YOLO-CBF~\cite{yolocbf_2023} & 37.20 & 104.5 & 95.60 & \textemdash & 95.10 & 99.00 & \textemdash & 12.40 & 80.6 \\
...
\midrule
\textbf{Rep-YOLO11s (Ours)} & 9.85 & 22.4 & \textbf{94.83} & \textbf{62.54} & \textbf{94.44} & \textbf{91.33} & \textbf{0.9190} & \textbf{2.14} & \textbf{467.3} \\
\bottomrule
\multicolumn{10}{l}{\small \textit{Note:} `\textemdash' indicates metrics not explicitly reported in the original publications.} \\
\multicolumn{10}{l}{\small Benchmarks evaluated on TensorRT 11.2 FP16 Engine on NVIDIA RTX 3050 Laptop GPU (Pure GPU forward inference time).}
\end{tabular}%
}
\end{table*}

\subsection{Dataset and Pre-processing}
We evaluate our method on the Safety Helmet Wearing Dataset (SHWD / VOC2028 format), comprising $7,581$ high-resolution construction site images ($6,064$ trainval images and $1,517$ test images). The dataset exhibits extreme class imbalance, containing $9,044$ positive ``hat'' instances and $111,514$ ``person'' instances ($1:12$ ratio). Label noise (e.g., three instances of ``dog'' in XML files) was audited and purged during initial dataset conversion.

\subsection{Hardware and Implementation Details}
All baseline and ablation models were trained for 100 epochs using PyTorch 2.5.1 and CUDA 12.4 with SGD optimizer (initial learning rate $\eta_0=0.01$, momentum $0.937$, weight decay $0.0005$, batch size $32$). Training was conducted on Kaggle Dual Tesla T4 GPUs (\textsf{device=0,1}). Deployment benchmarking was executed on an NVIDIA GeForce RTX 3050 Laptop GPU (4GB VRAM) using ONNX Runtime CUDA and TensorRT 11.2 FP16 engines.

\subsection{Comparison with State-of-the-Art (SOTA)}
Table~\ref{tab:sota_comparison} presents the quantitative benchmark results comparing Rep-YOLO11s against leading YOLO baselines and specialized helmet detection literature.

Rep-YOLO11s achieves superior detection fidelity ($mAP_{50} = 94.83\%$, $mAP_{50-95} = 62.54\%$, per-class ``hat'' recall of $91.33\%$, F1-score of $0.9190$). Thanks to TensorRT FP16 kernel fusion and structural re-parameterization, our model achieves a pure GPU inference latency of \textbf{2.14~ms}, corresponding to a peak throughput of \textbf{467.3~FPS}---substantially faster than competing methods.
...
$A_3$ & + ECA Attention & 94.79 & 62.48 & 90.95 & 6.64 \\
$A_4$ & + BiFormer Attention & 94.82 & 62.51 & 91.20 & 7.12 \\
$A_5$ & + Structural RepConv & 94.80 & 62.50 & 91.15 & \textbf{2.14}$^\star$ \\
$A_6$ & \textbf{Full Fusion (Proposed)} & \textbf{94.83} & \textbf{62.54} & \textbf{91.33} & \textbf{2.14}$^\star$ \\
\bottomrule
\multicolumn{6}{l}{\small Lat. = Pure GPU Latency (ms).} \\
\multicolumn{6}{l}{\small $^\star$ Evaluated after RepConv \textsf{switch\_to\_deploy} TensorRT FP16 conversion.}
\end{tabular}%
}
\end{table}

As shown in Table~\ref{tab:ablation_study}, inserting CoordConv ($A_2$) improves spatial coordinate awareness and feature localization. Adding BiFormer ($A_4$) improves helmet recall by $+0.32\%$. Finally, RepConv fusion ($A_5$) reduces latency from $7.12\text{~ms}$ to $2.14\text{~ms}$ without any loss in precision, providing an optimal efficiency-accuracy trade-off.
...
\begin{enumerate}
    ...
    \item \textbf{TensorRT GPU Inference ($T_{gpu}$)}: Pure FP16 model forward execution ($2.14\text{~ms}$).
...
In this paper, we presented \textbf{Rep-YOLO11s}, a structural re-parameterized object detector engineered for real-time safety helmet detection in complex construction environments. By integrating RepConv multi-branch fusion, Spatial CoordConv coordinate channels, BiFormer bi-level routing attention, and Focal EIoU Loss, Rep-YOLO11s addresses key industrial challenges including tiny distant target loss, class imbalance, and background clutter. Evaluated on the SHWD benchmark, Rep-YOLO11s achieves $94.83\%$ $mAP_{50}$ with a pure GPU inference latency of $2.14\text{~ms}$ ($467.3\text{~FPS}$) on TensorRT FP16, while maintaining an end-to-end RTSP pipeline throughput of $65\text{--}95\text{~FPS}$.
